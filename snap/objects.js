...
SpriteMorph.prototype.initBlocks = function () {
    SpriteMorph.prototype.blocks = {
		...
		brainWaveState:{
			type: 'predicate',
			category: 'RemoteCar',
			spec: 'brain wave state equals %carstate',
			default: []	
		},
		ballColor:{
			type: 'predicate',
			category: 'RemoteCar',
			spec: 'raspicam color %raspicolor',
			defaults: ['green']
		},
		boxAngle:{
			type: 'report',
		...
		ballAngle:{
			type: 'report',
			category: 'RemoteCar',
			spec: 'read ball angle',
			defaults: []
		},
		setGPIO:{
			type: 'command',
			category: 'RemoteCar',
			spec: 'set Pin %n to %motortor',
			defaults: []
		},
		setIPconnection:{
			type: 'command',
			category: 'RemoteCar',
			spec: 'set connceting IP %s',
			defaults:[localize('192.168.0.12:8080/')]
		},
		raspiultrasound:{
			type: 'reporter',
			category: 'RemoteCar',
			spec: 'ultradsound distance',
			default: [null]
		},
		//test mode for motor turn 
		// write ROM part
		setTyerSpeed:{
			type: 'command',
			category: 'RemoteCar',
			spec: 'Set %tyerside side tyer speed to %n',
			defaults: []
		},
		motorsetTorque:{
			only: SpriteMorph,
			type: 'command',
			category: 'RemoteCar',
			spec: 'set motor %n Torque %motortor',
			defaults: [null,'on']
		},
		motorsetmode: {
			only: SpriteMorph,
			type: 'command',
			category: 'RemoteCar',
			spec: 'set motor %n mode %motormode ',
			defaults: []
		},
		motorbroadcast:{
			only: SpriteMorph,
		...
		motorsetLED:{
			...
			type: 'command',
			category: 'RemoteCar',
			spec: 'set motor %n LED %motorcolor',
			defaults: [null]
		},
		motorsetID:{
			only: SpriteMorph,
		...
		motorsetposition:{
			...
			type: 'command',
			category: 'RemoteCar',
			spec: 'set ID %n to the position %n',
			defaults: [null,null]
		},
		motorclockwise: {
			only: SpriteMorph,
			type: 'command',
			category: 'RemoteCar',
			spec: '%clockwise turn motor %n by %n degrees',
			defaults: [null,15]
		},
		motorcounterclockwise: {
			...
			type: 'command',
			category: 'RemoteCar',
			spec: '%counterclockwise turn motor %n by %n degrees',
			defaults: [null,15]
		},
		motorsetvelocity: {
			only: SpriteMorph,
...
SpriteMorph.prototype.blockTemplates = function (category) {
    ...
    */
	...
	else if (cat === 'RemoteCar'){
		blocks.push(block('setIPconnection'));
		blocks.push('=');
        blocks.push(block('motorclockwise'));
		blocks.push(block('motorcounterclockwise'));
		
		blocks.push('-');
		blocks.push(block('setTyerSpeed'));
		blocks.push(block('motorsetposition'));
		blocks.push(block('motorsetvelocity'));
		
		blocks.push('=');
		blocks.push(block('motorsetLED'));
		blocks.push(block('motorbroadcast'));
		blocks.push(block('motorsetTorque'));
		blocks.push(block('motorsetID'));
		blocks.push(block('motorsetmode'));
		
	
		blocks.push('=');
		blocks.push(block('motorreadangle'));
		blocks.push(block('motorreadvelocity'));
		blocks.push(block('motorreadallangle'));
		blocks.push('-');
		blocks.push(block('motorallsetvelocity'));
		
		blocks.push('=');
		blocks.push(block('raspiultrasound'));
		blocks.push(block('setGPIO'));
		blocks.push(block('checkBall'));
		blocks.push(block('checkBox'));		
		blocks.push(block('ballAngle'));
		blocks.push(block('boxAngle'));
...
// SpritMorph raspiCar readallangle
