...
Process.prototype.setIPconnection =  function(IP){
	this.IP = IP;
...
Process.prototype.motorsyncwrite = function(body){
	objSequence = body.blockSequence();
	objLength = objSequence.length;
	objName = objSequence[0].selector;
	var i;
	state = true;
	for (i=1;i<objLength;i++){
		if(objName == objSequence[i].selector){
			
		}else{	
			state = false;
			console.log('input wrongs');
		}
	}
	if(state){
	// 假設輸入參數為兩個
	var paramID = objSequence[0].inputs()[0].children[0].text;
	var paramData = objSequence[0].inputs()[1].children[0].text;
	
	for (i=1; i<objLength;i++){
		paramID += "/" + objSequence[i].inputs()[0].children[0].text;
		paramData += "/" + objSequence[i].inputs()[1].children[0].text;
	}
	// send command to raspberry
	state = "sync";
	IP = this.IP;
	URL = IP + state+"?"+objName+","+paramID+ "," +paramData;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
// Process raspberry pi3 GPIO
Process.prototype.setGPIO = function(pin, command){
	state = "setGPIO";
	IP = this.IP;
	URL = IP + state+"?"+pin+ "," +command;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
// Process raspberry picam for catch ball
Process.prototype.checkBall = function(){
...
Process.prototype.checkBox = function(){
...
Process.prototype.ballColor = function(color){
...
Process.prototype.boxAngle = function(){
...
Process.prototype.ballAngle = function(){
...
// Process motor primitives
Process.prototype.motorsetLED = function(ID,color){
	state = "motorsetLED";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +color;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
Process.prototype.motorbroadcast = function(){

	state = "motorbroadcast";
	IP = this.IP;
	URL = IP + state+"?";
	console.log(URL);
	//send
...
Process.prototype.raspiultrasound = function(){
	state = "raspiultrasound";
	IP = this.IP;
	URL = IP + state+"?";
	console.log(URL);
	var response;
...
Process.prototype.motorreadallangle = function(){

	state = "motorreadallangle";
	IP = this.IP;
	URL = IP + state+"?";
	console.log(URL);
	//send
...
Process.prototype.motorclockwise = function(ID,degrees){



	state = "motorclockwise";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +degrees;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
Process.prototype.motorsetposition = function(ID,degrees){


state = "motorsetposition";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +degrees;
	console.log(URL);
	//send
...
Process.prototype.motorcounterclockwise = function(ID,degrees){
	 
		
	  state = "motorcounterclockwise";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +degrees;
	console.log(URL);
	//send
...
Process.prototype.motorsetTorque = function(ID,state){

	  state = "motorsetTorque";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +state;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
Process.prototype.motorsetmode = function(ID,data){

	state = "motorsetmode";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +data;
	console.log(URL);
	//send
	var response;
    if (!this.httpRequest) {
...
Process.prototype.motorsetID = function(data){
...
Process.prototype.setTyerSpeed = function(side,speed){
...
Process.prototype.motorsetvelocity = function(ID,data){

	state = "motorsetvelocity";
	IP = this.IP;
	URL = IP + state+"?"+ID+ "," +data;
	console.log(URL);
	//send
...
Process.prototype.motorallsetvelocity = function(data){

	state = "motorallsetvelocity";
	IP = this.IP;
	URL = IP + state+"?"+data;
	console.log(URL);
	//send
...
Process.prototype.motorreadvelocity = function(ID){
			

	state = "motorreadvelocity";
	IP = this.IP;
	URL = IP + state+"?"+ID;
	console.log(URL);
	//send
...
Process.prototype.motorreadangle = function (ID) {
		

	state = "motorreadangle";
	IP = this.IP;
	URL = IP + state+"?"+ID;
	console.log(URL);
	//send
